...
def fetch_data() -> tuple[pd.DataFrame, float | None]:
    ...
    for run in runs:
        if run.id in COOPERATIVE_RUNS or run.name in COOPERATIVE_RUNS:
            label = COOPERATIVE_RUNS.get(run.id, COOPERATIVE_RUNS.get(run.name))
            ...
            merged_df[ROUNDS_METRIC] = merged_df[ROUNDS_METRIC].ffill().fillna(0)
...
def _accuracy_formatter(max_value: float) -> tuple[FuncFormatter, str]:
    if max_value <= 1.5:
        return FuncFormatter(lambda y, _pos: f"{100 * y:.0f}"), "Test accuracy (%)"
