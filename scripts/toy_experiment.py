...
def main(cfg: DictConfig) -> None:
    ...
    # Per-agent overrides: prefer model.agents (nested style as in the 2-agent
    # config) and fall back to top-level agents (flat style as in the 30-agent
    # config).
    _model_agents = OmegaConf.select(cfg.model, 'agents')
    if _model_agents is not None:
        per_agents_cfg_raw = OmegaConf.to_container(_model_agents, resolve=True)
    else:
    ...
    for i in range(n_agents):
        ...
        # Strip the nested agents key so it is not forwarded to the model
        # constructor as an unexpected keyword argument.
    ...
    # NOTE: each agent has its own validation set (different class distribution
    # when split_strategy != 'uniform'), so these numbers are NOT comparable
    # across agents — treat them as per-agent input complexity estimates.
    ...
    print('\nExtracting training latents for whitening...')
    ...
    print('Extracting pilot latents for alignment...')
    ...
    print('Fitting pairwise alignment matrices...')
