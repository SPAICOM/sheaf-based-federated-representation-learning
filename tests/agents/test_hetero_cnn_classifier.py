"""Tests for src.agents.hetero_cnn_classifier."""
...
from src.agents.utils import HeteroCNN


class TestHeteroCNNClassifier:
    def _make(self, **kwargs) -> HeteroCNNClassifier:
        return HeteroCNNClassifier(
            **{'in_features': 3, 'num_classes': 10, 'rate': 0.5, **kwargs}
        )
    ...
    def test_encoder_is_cnn(self):
        assert isinstance(self._make().encoder, HeteroCNN)

    # --- construction ---

    def test_default_encoder_hidden_dims(self):
        """Default encoder uses [32, 64, 128];
        Rate is 0.5, out_features is 64."""
        agent = self._make()
        assert agent.encoder.out_features == 64

    def test_custom_encoder_hidden_dims(self):
        agent = self._make(encoder_hidden_dims=[16, 32])
        assert agent.encoder.out_features == 16
    ...
    def test_encode_custom_dims(self):
        agent = self._make(encoder_hidden_dims=[16, 32])
        assert agent.encode(self._x()).shape == (4, 16)
